...
trait SparkSessionTrait {
  ...
  lazy val spark: SparkSession =
    try {
      ...
      case e: Throwable => throw new Exception("Failed to create a Spark Session: "+e.getMessage)
    }
  ...
  lazy val fileSystem: FileSystem = {
    try {
      ...
      case e: Throwable => throw new Exception("Failed to create a FileSystem: "+e.getMessage)
    }
  }

  object SparkSessionTrait {
    def apply(iAppName: String, iSparkConf: SparkConf = new SparkConf(), iMaster: String = "local[*]"): Unit = {
